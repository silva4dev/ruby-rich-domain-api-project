...
class OrderItemTest < Minitest::Test
  def setup
    ...
    @price = 50.0
    @quantity = 2
    @order_item = Checkout::Domain::Entity::OrderItem.new(@id_item, @price, @quantity)
  end
  ...
  def test_initialize_with_invalid_quantity
    assert_raises(ArgumentError) do
      Checkout::Domain::Entity::OrderItem.new(@id_item, @price, 0)
    end

    assert_raises(ArgumentError) do
      Checkout::Domain::Entity::OrderItem.new(@id_item, @price, -1)
    end
  end
